module HasManyWithSet
  ...
  def has_many_with_set (child, class_name)
    build_set_relationship self.to_s.split("::").last,self.to_s, child.to_s.classify, class_name.to_s
  end

  private

  def build_set_relationship (parent_model_name, parent_class_name, child_model_name, class_name)
    ...
    Relationships.relate_parent_to_set(set_model_name, parent_model_name,parent_class_name)
